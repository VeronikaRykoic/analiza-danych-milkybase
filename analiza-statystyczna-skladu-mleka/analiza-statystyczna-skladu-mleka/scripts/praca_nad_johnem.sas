...
/* Analiza normalności dla WSZYSTKICH próbek (bez filtrowania) */
ods graphics on;
proc univariate data=work.dane_finalne normal;
    var Protein Fat Lactose;
    histogram / normal;
    qqplot / normal(mu=est sigma=est);
run;
ods graphics off;
/* Filtrowanie do pierwszego oddania mleka i podział na grupy skrajne (<= 25 oraz >= 35) */
data work.dane_ekstremalne;
    set work.dane_finalne; 
    
    where Subject_Deposit_Order = 1; 
    
    if Mom_Age <= 25 then Grupa = "1_Mlodzsze_25"; 
    else if Mom_Age >= 35 then Grupa = "2_Starsze_35"; 
    else delete; 
run;

/* Obliczenie i wyświetlenie podstawowych statystyk opisowych dla nowych grup wiekowych */
proc means data=work.dane_ekstremalne n mean std;
    class Grupa;
    var Protein Lactose Fat;
run;

/* Porównanie wartości makroskładników między nowymi grupami za pomocą testu t-Studenta */
proc ttest data=work.dane_ekstremalne;
    class Grupa;
    var Protein Lactose Fat;
run;